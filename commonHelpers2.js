...
</div>`;Q.innerHTML=t};async function he(){try{const t=localStorage.getItem("products-home-page-filters");if(t){const e=JSON.parse(t);e.length>=d?g=e.slice(0,d):(await P(),g=m.results,localStorage.setItem("products-home-page-filters",JSON.stringify(g)))}else await P(),g=m.results,localStorage.setItem("products-home-page-filters",JSON.stringify(g));O(g),m.totalPages===0&&f()}catch(t){f(),console.error("Error:",t.message)}}he();ue.addEventListener("submit",async t=>{t.preventDefault(),$=me.value.trim(),M($,p,v,d),await P(),C=m.results,O(C),m.totalPages===0&&f()});async function ye(){try{const t=localStorage.getItem("categories-filters");if(t)L=JSON.parse(t);else{const e="",s=`${G}/categories`;L=await new B(s,e,v,d).fetchBreeds(),localStorage.setItem("categories-filters",JSON.stringify(L))}Se(L)}catch(t){f(),console.error("Error:",t.message)}}ye();function Se(t){const e=[];t.forEach(c=>{let o="";c!=="Pantry_Items"?o=`<li class="li-first-select-search"><button class="button-li-filters">${c.replace(/_/g," ").replace(/&/g,"/")}</button></li>`:o=`<li class="li-first-select-search"><button class="button-li-filters">${c}</button></li>`,e.push(o)}),e.push('<li class="li-first-select-search"><button class="button-li-filters">Show all</button></li>'),W.insertAdjacentHTML("beforeend",e.join(""));const s=document.querySelectorAll(".button-li-filters");Le(s)}J.addEventListener("click",()=>ve(J,W));function ve(t,e){e.classList.add("first-select-search"),document.addEventListener("click",s=>be(s,t,e))}function be(t,e,s){!e.contains(t.target)&&!s.contains(t.target)?s.classList.remove("first-select-search"):s.contains(t.target)&&setTimeout(()=>{s.classList.remove("first-select-search")},100)}function Le(t){t.forEach(e=>{e.addEventListener("click",$e)})}async function $e(t){const e=t.currentTarget.textContent;e!=="Pantry Items"?p=e.replace(/ /g,"_").replace(/\//g,"&"):p=e,pe.innerHTML=`${e}`,p==="Show_all"&&(p=""),M($,p,v,d),await P(),C=m.results,O(C),m.totalPages===0&&f()}function O(t){const e=[],s=JSON.parse(localStorage.getItem("cart"));t.forEach(o=>{let a="",r="icon-heroicons-solid_shopping-cart",i=o.category.replace(/_/g," ").replace(/&/g,"/");o.category=="Pantry_Items"?i=o.category:i=o.category.replace(/_/g," ").replace(/&/g,"/"),s&&(s.some(h=>h._id===o._id)?r="icon-check":r="icon-heroicons-solid_shopping-cart"),o.is10PercentOff?a=`<li class="card-list-item id-for-del" data-id=${o._id}>
                <div class = "div-img">
