...
</div>`;R.innerHTML=t};async function ue(){try{const t=localStorage.getItem("products-home-page-filters");if(t){const e=JSON.parse(t);e.length>=d?g=e.slice(0,d):(await C(),g=u.results,localStorage.setItem("products-home-page-filters",JSON.stringify(g)))}else await C(),g=u.results,localStorage.setItem("products-home-page-filters",JSON.stringify(g));M(g),u.totalPages===0&&p()}catch(t){p(),console.error("Error:",t.message)}}ue();ae.addEventListener("submit",async t=>{t.preventDefault(),P=ie.value.trim(),_(P,m,S,d),await C(),$=u.results,M($),u.totalPages===0&&p()});async function me(){try{const t=localStorage.getItem("categories-filters");if(t)L=JSON.parse(t);else{const e="",s=`${Q}/categories`;L=await new k(s,e,S,d).fetchBreeds(),localStorage.setItem("categories-filters",JSON.stringify(L))}ge(L)}catch(t){p(),console.error("Error:",t.message)}}me();function ge(t){const e=[];t.forEach(c=>{let o="";c!=="Pantry_Items"?o=`<li class="li-first-select-search"><button class="button-li-filters">${c.replace(/_/g," ").replace(/&/g,"/")}</button></li>`:o=`<li class="li-first-select-search"><button class="button-li-filters">${c}</button></li>`,e.push(o)}),e.push('<li class="li-first-select-search"><button class="button-li-filters">Show all</button></li>'),U.insertAdjacentHTML("beforeend",e.join(""));const s=document.querySelectorAll(".button-li-filters");ye(s)}N.addEventListener("click",()=>pe(N,U));function pe(t,e){e.classList.add("first-select-search"),document.addEventListener("click",s=>fe(s,t,e))}function fe(t,e,s){!e.contains(t.target)&&!s.contains(t.target)?s.classList.remove("first-select-search"):s.contains(t.target)&&setTimeout(()=>{s.classList.remove("first-select-search")},100)}function ye(t){t.forEach(e=>{e.addEventListener("click",he)})}async function he(t){const e=t.currentTarget.textContent;e!=="Pantry Items"?m=e.replace(/ /g,"_").replace(/\//g,"&"):m=e,ne.innerHTML=`${e}`,m==="Show_all"&&(m=""),_(P,m,S,d),await C(),$=u.results,M($),u.totalPages===0&&p()}function M(t){const e=[],s=JSON.parse(localStorage.getItem("cart"));t.forEach(o=>{let r="",a="icon-heroicons-solid_shopping-cart",i=o.category.replace(/_/g," ").replace(/&/g,"/");o.category=="Pantry_Items"?i=o.category:i=o.category.replace(/_/g," ").replace(/&/g,"/"),s&&(s.some(f=>f._id===o._id)?a="icon-check":a="icon-heroicons-solid_shopping-cart"),o.is10PercentOff?r=`<li class="card-list-item id-for-del" data-id=${o._id}>
                <div class = "div-img">
